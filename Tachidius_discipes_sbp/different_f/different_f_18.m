...
data.tL18 = [t_data, Lmean]; % d, cm- time, mean length
...
xlabel('Time (days)');
ylabel('Length (cm)');
title('Model Predictions for Different f Values at 18  C');
legend('show');
hold on
scatter(t_data,Lmean);
% Define the time period over which you want to compute the average growth rate
time_period_start = 5; % Start of the time period (in days)
time_period_end = 15; % End of the time period (in days)
